...
public class ProductResourceITest {
	...
	public void findAllShoudReturnSortedPageWhenSortByName() throws Exception {
		...
		result.andExpect(jsonPath("$.totalElements").value(totaldeAProdutos));
	...
	public void  updateProductDTOQuandoIdExistente() throws Exception {
		...
		String excpectedName = productDTO.getName();
		ResultActions  reuslt = mockMvc.perform(put("/products/{id}", existId)
				.contentType(MediaType.APPLICATION_JSON)
				.content(jsonBody)
				.accept(MediaType.APPLICATION_JSON));
		
		reuslt.andExpect(status().isOk());
		reuslt.andExpect(jsonPath("$.id").value(existId));
		reuslt.andExpect(jsonPath("$.name").value(excpectedName));
	}
	
	@Test
	public void  updateDeveRetornarNotFoundIdNaoExiste() throws Exception {
		String jsonBody = mapper.writeValueAsString(productDTO);
